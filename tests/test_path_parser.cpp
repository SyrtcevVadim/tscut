...
{
    request_processing::cut_video_request request;
    SECTION("Cut video from start")
    {
        std::string request_path{"/ts/mdb4/my_folder/abcd/1231324343.ts/0/3477"};
        REQUIRE(request_processing::parse_cut_video_request(request_path, request));
        ...
        REQUIRE(request.video_duration_ms == 3477);
    }

    SECTION("Path hasn't file with extension")
    {
        std::string request_path{"/ts/mdb4/my_folder/abcd/1231324343/0/3477"};
        REQUIRE_FALSE(request_processing::parse_cut_video_request(request_path, request));
    }
}
