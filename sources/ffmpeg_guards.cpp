...
namespace ffmpeg {
  ...
  AVPacket* AVPacketWrapper::get_ptr() {
    return packet;
  }
  
  bool AVPacketWrapper::is_valid() {
    return (packet != nullptr)?true:false;
  }
  ...
  AVInputFormatContextWrapper::AVInputFormatContextWrapper(std::string &path_to_input_file) {
  ...
  AVOutputFormatContextWrapper::AVOutputFormatContextWrapper(std::string path_to_output_file) {
    this->path_to_output_file = path_to_output_file;
    avformat_alloc_output_context2(&output_context, nullptr, nullptr, path_to_output_file.c_str());
  }
