...
namespace request_processing 
{
  bool parse_cut_video_request(std::string &request_path, cut_video_request &cut_video_request)
  {
    /*
      ...
      ^/(?<video_format>[^/]+)/ - именованная группа для сохранения формата видео из url. Пример: /ts/
      знак ^ указывает на начало URL-строки, а шаблон [^/]+ позволяет выбрать все символы, кроме символа косой черты, т.к. данный символ является разделителем
      (?<path_to_video>.+\.[^/]+)/ - именованная группа для сохранения пути к видео из url.
